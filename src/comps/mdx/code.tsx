...
export async function Code(props: Props) {
  ...
  if (isInline) {
    return (
      <code
        {...props}
        className="
          -my-1 rounded-md border border-gray-400/50 bg-white/30 px-1.5 py-1 font-mono text-[0.9em]
          font-semibold text-current shadow-purple-300/5
          before:content-none
          after:content-none
          dark:border-white/10 dark:bg-gray-600/25 dark:font-normal
        "
      />
    )
  }
  ...
  return (
    <div
      className="
        not-prose border-beige/30 shadow-beige/20 relative my-8 rounded-md border text-sm leading-6
        font-normal [box-shadow:0_0_24px_var(--tw-shadow-color)]
    ...
    >
      ...
      {lang && !hiddenLangs.includes(lang) && <span className={`
        text-2xs absolute top-0 right-0 rounded-tr rounded-bl px-1.5 pt-1 pb-1 leading-none
        font-bold text-[#1a1b26] uppercase select-none
        ${colors[lang] || `bg-[#BB9AF7]/80`}
          `}>{lang}</span>}
    </div>
  )
}
